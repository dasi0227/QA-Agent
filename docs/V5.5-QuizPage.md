...
POST /practice/qaSetEntry/save
POST /practice/qaSetEntry/unknown
POST /practice/qaSetEntry/answer
...
```json
{
  ...
  "items": [
    {
      "sessionItemId": "session-qaSetEntry-id",
      "qaItemId": "qa-qaSetEntry-id",
      "sortOrder": 1,
      "question": "题干",
      "knowledgeNote": "知识笔记",
      "standardAnswer": "标准答案",
...
```text
AnswerEditor 输入
  -> debounce qaSetEntry/save
  -> qaSetEntry.status = DRAFT
  -> 更新保存状态
```
...
1. 修改数据库结构：补 session、session qaSetEntry、题目快照。
...
4. `qaSetEntry/save` 不触发反馈，只保存草稿。
5. `qaSetEntry/answer` 能保存 feedback。
6. `qaSetEntry/unknown` 能保存 UNKNOWN。
