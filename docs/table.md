...
本文定义 `QA_Agent` 第一版正式数据库设计，只服务当前已经确认的产品主闭环：
...
1. 第一版 `file_type` 固定只支持 `MARKDOWN`。
2. `summary` 表示资料级摘要。
...
1. `document_ids_json` 第一版先保留，不再单独拆任务资料关系表。
2. `qa_set_id` 用于指向成功产出的问答集，可为空。
3. 第一版不保留 `progress`。
4. 第一版不在主表中保留 `message`，完整阶段消息交由独立消息表承载。
...
1. 第一版不保留 `tags_json`。
2. 第一版不把题目轻量统计挂在题目表。
3. `source_chunk_ids_json` 第一版先保留在题目表中，不单独拆证据关系表。
...
1. 第一版保留用户作答文本 `user_answer`。
2. 第一版不保留 `is_unknown`，由 `result` 承载“不会”语义。
...
1. 真正长期业务关系，用表表达
2. 第一版只保留一个必要的多对多关系表：`qa_set_document_ref`
3. 任务消息流单独建表，不塞回主任务表
4. `RAG` 以 `document_chunk` 为正式数据底座
5. 不提前引入 `Memory`
6. 不提前做复杂统计和复杂审计体系
7. 先把 Agent 的创建和测试主闭环做稳
