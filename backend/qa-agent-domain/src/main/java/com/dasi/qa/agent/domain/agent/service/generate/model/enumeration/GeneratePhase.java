...
public enum GeneratePhase {
    ...
    DECIDE("DECIDING", "DecideAgent", "识别用户请求是否满足问答集生成场景并给出判定结果。", "decideResult"),
    ABORT("ABORTING", "AbortAgent", "根据拒绝原因生成终止消息并结束当前生成任务。", null),
    PLAN("PLANNING", "PlanAgent", "分析资料摘要并输出模块化题量与难度分配计划。", "planResult"),
    WRITE("WRITING", "WriteAgent", "顺序执行规划、起草、校验修订与总结，生成最终可落库问答集。", "draftResult"),
    DRAFT("DRAFTING", "DraftAgent", "基于检索证据按模块起草结构化问答题目。", null),
    VALIDATE("VALIDATING", "ValidateAgent", "评估并修订候选问答，确保质量达标。", "validateResult"),
    EVALUATE("EVALUATING", "EvaluateAgent", "审校题目准确性、完整性与证据边界并输出判定。", null),
